### Problemas Básicos
- [x] Par ou Impar
- [x] Tabela Multiplicação
- [x] Soma dos Números Naturais
- [x] Soma dos Números Naturais ao Quadrado
- [x] Troca de Variáveis
- [x] Número Mais Próximo
- [x] Problema dos Dados
- [x] Enésimo termo de uma PA

### Problemas Fáceis
- [x] Soma dos Dígitos
- [x] Inversão dos Dígitos
- [x] Teste dos Número Primos
- [x] Teste de Potência
- [x] Distância Entre Dois Pontos
- [x] Verificação de Triângulo
- [x] Sobreposição de Quadrados
- [x] Fatorial de um Número
- [x] Soma de Cubos de Pares
- [x] Maior Divisor Comum & Maior Fator Comum
- [x] Minímo Múltiplo Comum
- [x] Número Perfeito
- [x] Soma de Frações
- [x] Dia da Semana
- [x] Enésimo Número de Fibonacci
- [x] Decimal para Binário
- [x] Enésimo Número da Sequência 1, 3, 5, ...
- [x] Número de Armstrong
- [x] Palíndromo
- [x] Raíz do Dígito

### Problemas Intermediários
- [x] Raiz Quadrada
- [x] 3 Divisores
- [x] Divisível por 4
- [x] Divisibilidade por 11
- [x] Divisibilidade por 13
- [x] Kth dígito em a^b
- [x] Fração para Decimal Recorrente
- [x] Sequência Recorrente em uma Fração
- [x] Compute nPr
- [x] Compute nCr
- [x] Triângulo de Pascal
- [x] Todos Fatores
- [x] Fatores Primos
- [x] Maior Fator Primo
- [x] Exponenciação Modular
- [x] Nth Número Catalão
- [x] Coeficiente Binomial
- [x] Conjunto Potência
- [x] Próxima Permutação

### Problemas Difíceis

- [x] Crivo de Eratosthenes
- [x] Super Primo
- [x] Problema do Ângulo do Relógio
- [x] Torre de Hanoi
- [ ] Rato e o Veneno
- [ ] Problema dos 8 Puzzle
- [ ] Função do Totem de Euler
- [ ] Problema de Josephus
